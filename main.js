      ...
      return new Promise(async (resolve, reject) => {
         ...
         if (course.settings['Create three main folders']) {
            try {
               ...
               if (!documents) documents = await canvasCourse.folders.create({
                  name: 'documents',
                  parent_folder_id: topFolder.id
               });
               if (!media) media = await canvasCourse.folders.create({
                  name: 'media',
                  parent_folder_id: topFolder.id
               });
               if (!template) template = await canvasCourse.folders.create({
                  name: 'template',
                  parent_folder_id: topFolder.id
               });
         ...
         if (course.settings['Create Archive']) {
            try {
               ...
               if (!archive) archive = await canvasCourse.folders.create({
                  name: 'archive',
                  parent_folder_id: topFolder.id
               });
               course.log('Folders Created', {
